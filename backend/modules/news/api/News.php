 ...
 */
class News extends \backend\components\API
{
    ...
    public function api_items($options = [])
    {
        if (! $this->_items) {
            ...
            if (! empty($options['orderBy'])) {
                ...
                $query->orderBy(['sequence' => SORT_DESC, 'time' => SORT_DESC]);
            }
    ...
    public function api_last($limit = 1)
    {
        ...
        $result = [];
        foreach (NewsModel::find()->with($with)
            ->status(NewsModel::STATUS_ON)
            ->sortDate()
            ->limit($limit)
            ->all() as $item) {
    ...
    {
        return $this->_adp ? LinkPager::widget([
    ...
    {
        $news = NewsModel::find()->where(['or', 'news_id=:id_slug', 'slug=:id_slug'], [':id_slug' => $id_slug])
            ->status(NewsModel::STATUS_ON)
            ->one();
        if ($news) {
