...
use backend\models\CopyModuleForm;
...
class ModulesController extends \backend\components\Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                if($model->save()){
                    $this->flash('success', Yii::t('backend', 'Module created'));
    ...
    public function actionEdit($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                if($model->save()){
                    $this->flash('success', Yii::t('backend', 'Module updated'));
                }
                else{
    ...
    public function actionSettings($id)
    {
        ...
        if (Yii::$app->request->post('Settings')) {
            ...
            if($model->save()){
                $this->flash('success', Yii::t('backend', 'Module settings updated'));
            }
            else{
                $this->flash('error', Yii::t('backend', Yii::t('backend', 'Update error. {0}', $model->formatErrors())));
            }
            return $this->refresh();
        }
        else {
    ...
    public function actionRestoreSettings($id)
    {
        if(($model = Module::findOne($id))){
            $model->settings = '';
            $model->save();
            $this->flash('success', Yii::t('backend', 'Module default settings was restored'));
        } else {
            $this->flash('error', Yii::t('backend', 'Not found'));
        }
        return $this->back();
    ...
    public function actionCopy($id)
    {
        ...
        if ($formModel->load(Yii::$app->request->post())){
            if(Yii::$app->request->isAjax){
                ...
                $moduleFileContent = str_replace('@backend', '@app', $moduleFileContent);
                ...
                foreach(new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($newModuleFolder)) as $file => $object){
                    if(!$object->isDir()){
                        ...
                        $fileContent = str_replace("Yii::t('backend/".$module->name, "Yii::t('backend/".$formModel->name, $fileContent);
                ...
                foreach(glob($newModuleFolder . DIRECTORY_SEPARATOR . 'models' . DIRECTORY_SEPARATOR . '*.php') as $modelFile){
                    ...
                    $newTableName = str_replace('backend', 'app', $newTableName);
    ...
    public function actionDelete($id)
    {
        ...
        return $this->formatResponse(Yii::t('backend', 'Module deleted'));
    }
